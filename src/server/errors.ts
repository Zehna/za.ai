 ...
 * so every failure surfaces as { error: { code, message } }. All messages are
 * passed through the secret redactor before reaching clients or logs.
 */
export function registerErrorHandling(
  app: FastifyInstance,
  options: { redact: SecretRedactor },
): void {
  ...
  app.setErrorHandler((error: FastifyError, _request, reply) => {
    if (error instanceof ValidationError) {
      return reply
        .status(400)
        .send({ error: { code: "invalid_request", message: redact(error.message) } });
    }
    if (error instanceof ConversationNotFoundError) {
      return reply
        .status(404)
        .send({ error: { code: "not_found", message: redact(error.message) } });
    }
    if (error instanceof ProviderError || error instanceof ConfigError) {
      app.log.error({ err: redact(error.message) }, "provider failure");
      return reply
        .status(502)
        .send({ error: { code: "provider_error", message: redact(error.message) } });
    }
    ...
    app.log.error({ err: redact(error.message) }, "unexpected server error");
